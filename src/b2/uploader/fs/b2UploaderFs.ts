import { envB2ObjectStorageKeyIdResult } from "@/app/env/private/envB2ObjectStorageKeyIdResult"
import { envB2ObjectStorageKeyResult } from "@/app/env/private/envB2ObjectStorageKeyResult"
...
export async function b2UploaderFs(): PromiseResult<B2UploaderFn> {
  ...
  const keyIdResult = envB2ObjectStorageKeyIdResult()
  if (!keyIdResult.success) return keyIdResult
  const keyId = keyIdResult.data

  const keyResult = envB2ObjectStorageKeyResult()
